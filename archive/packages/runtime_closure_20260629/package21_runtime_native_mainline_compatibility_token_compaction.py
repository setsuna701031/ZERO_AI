...
REPORT_PATH = ROOT / "runtime_native_mainline_compatibility_token_compaction_report.txt"
...
MARKER_BEGIN = "# ZERO_PACKAGE21_RUNTIME_NATIVE_COMPATIBILITY_TOKEN_COMPACTION_BEGIN"
MARKER_END = "# ZERO_PACKAGE21_RUNTIME_NATIVE_COMPATIBILITY_TOKEN_COMPACTION_END"
...
try:
    ...
    def _zero_runtime_native_mainline_admit_legacy_request(self, payload):
        return _zero_runtime_native_entry_adapter.run_runtime_native_entry(self, payload)

    def _zero_runtime_native_mainline_normalize_legacy_request(self, payload):
        return _zero_runtime_native_entry_adapter.normalize_runtime_native_entry_request(payload)
...
def _backup(path: Path) -> None:
    ...
    shutil.copy2(path, path.with_name(f"{path.name}.package21_backup_{stamp}"))
...
def _refresh_marker_tests() -> None:
    replacements = [
        ("ZERO_PACKAGE18_RUNTIME_NATIVE_COMPATIBILITY_RETIREMENT_BEGIN", "ZERO_PACKAGE21_RUNTIME_NATIVE_COMPATIBILITY_TOKEN_COMPACTION_BEGIN"),
        ("ZERO_PACKAGE18_RUNTIME_NATIVE_COMPATIBILITY_RETIREMENT_END", "ZERO_PACKAGE21_RUNTIME_NATIVE_COMPATIBILITY_TOKEN_COMPACTION_END"),
        ("ZERO_PACKAGE19_RUNTIME_NATIVE_COMPATIBILITY_RETIREMENT_PHASE2_BEGIN", "ZERO_PACKAGE21_RUNTIME_NATIVE_COMPATIBILITY_TOKEN_COMPACTION_BEGIN"),
        ("ZERO_PACKAGE19_RUNTIME_NATIVE_COMPATIBILITY_RETIREMENT_PHASE2_END", "ZERO_PACKAGE21_RUNTIME_NATIVE_COMPATIBILITY_TOKEN_COMPACTION_END"),
    ]
    for rel in [
...
def main() -> int:
    ...
    if normalize_count > 1 or run_count > 1:
        raise RuntimeError(
            f"Unexpected token counts after compaction: normalize={normalize_count}, run={run_count}"
        )

    report = "\n".join([
        "Package21 Runtime Native Mainline Compatibility Token Compaction Report",
        "",
        f"root: {ROOT}",
        ...
        "",
        "Completed:",
        "- replaced Package19 active marker with Package21 token-compacted bridge",
        "- changed mainline bridge to import runtime_native_entry_adapter as a module alias",
        "- reduced direct mainline references to normalize_runtime_native_entry_request and run_runtime_native_entry",
        "- refreshed Package18/19 marker tests to recognize Package21 as active",
        "",
