...
from cli.zero_capability_strategy_bootstrap_runtime_integration_boundary import run
...
def _write_consumption(path):
    ...
    path.write_text(json.dumps(consumption), encoding="utf-8")
    return consumption


def test_cli_integrate_validate_inspect_and_does_not_modify_input(tmp_path):
    source = tmp_path / "consumption.json"; consumption = _write_consumption(source); original = source.read_bytes()
    boundary, code = run(["integrate", str(source)])
    assert code == 0 and boundary["status"] == "integrated"
    artifact = tmp_path / "boundary.json"; artifact.write_text(json.dumps(boundary), encoding="utf-8")
    ...
    assert code == 0 and inspection["source_consumption_id"] == consumption["consumption_id"]
...
def test_cli_validation_failure_invalid_json_and_missing_file(tmp_path):
    ...
    for path in (invalid, tmp_path / "missing.json"):
        result, code = run(["integrate", str(path)])
...
def test_cli_subprocess_nonzero_without_traceback(tmp_path):
    ...
    completed = subprocess.run([sys.executable, "-m", "cli.zero_capability_strategy_bootstrap_runtime_integration_boundary", "integrate", str(source)], capture_output=True, text=True, encoding="utf-8")
