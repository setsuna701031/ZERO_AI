...
def _config(tmp_path: Path, **overrides):
    data = {
        ...
        "max_tick_limit": 3,
        "checkpoint_path": str(tmp_path / "gate-checkpoint.json"),
...
def _approval(tmp_path: Path) -> dict[str, object]:
    launch = launch_goal_session("executor invocation gate", _config(tmp_path))
    ...
    return evaluate_executor_invocation_approval(preparation)


def test_valid_approval_opens_gate_metadata_only(tmp_path: Path) -> None:
    approval = _approval(tmp_path)
    result = evaluate_executor_invocation_gate(approval)

    assert result["schema"] == ZERO_RUNTIME_EXECUTOR_INVOCATION_GATE_SCHEMA
    ...
    assert result["source_executor_invocation_approval_id"] == approval[
        "executor_invocation_approval_id"
    ]
    assert result["executor_invocation_approval_id"] == approval[
        "executor_invocation_approval_id"
    ]
    assert result["denial_reason"] == ""


def test_rejected_approval_is_rejected(tmp_path: Path) -> None:
    approval = _approval(tmp_path)
    approval["invocation_approval_status"] = "rejected"
    approval["executor_invocation_approved"] = False

    result = evaluate_executor_invocation_gate(approval)

    assert result["invocation_gate_status"] == "rejected"
    assert result["executor_invocation_gate_open"] is False
    assert result["denial_reason"] == "approval_not_approved"


def test_duplicate_gate_is_rejected(tmp_path: Path) -> None:
    approval = _approval(tmp_path)
    first = evaluate_executor_invocation_gate(approval)
    result = evaluate_executor_invocation_gate(
        approval,
        existing_gates=[first],
    )

    assert result["invocation_gate_status"] == "rejected"
    assert result["executor_invocation_gate_open"] is False
    assert result["denial_reason"] == "duplicate_invocation_gate"
    assert result["executor_invoked"] is False
...
def test_lineage_is_preserved(tmp_path: Path) -> None:
    ...
    for field in (
        ...
        assert result[field] == approval[field]


def test_gate_does_not_invoke_executor_or_start_execution(tmp_path: Path) -> None:
    service = RuntimeOperatorService(_config(tmp_path))
    result = service.run_goal("operator invocation gate")
    status = service.status()
    gate = result["executor_invocation_gate"]
    ...
    assert gate["executor_invocation_gate_open"] is True
    assert gate["executor_invoked"] is False
    assert gate["execution_started"] is False
    assert gate["runtime_mutated"] is False
    assert status["invocation_gate_status"]["invocation_gate_status"] == "opened"
    assert status["invocation_gate_status"]["executor_invocation_gate_open"] is True
    assert status["invocation_gate_status"]["executor_invoked"] is False
    assert status["invocation_gate_status"]["execution_started"] is False
...
def test_summary_is_operator_visible_and_safe(tmp_path: Path) -> None:
    result = evaluate_executor_invocation_gate(_approval(tmp_path))
    summary = summarize_executor_invocation_gate(result)

    assert summary["invocation_gate_status"] == "opened"
    assert summary["executor_invocation_gate_open"] is True
    assert summary["executor_invoked"] is False
    assert summary["execution_started"] is False
    assert summary["runtime_mutated"] is False
    assert summary["executor_invocation_gate_id"]


def test_source_boundary_has_no_forbidden_execution_surfaces() -> None:
    source = Path("core/runtime/runtime_executor_invocation_gate.py").read_text(
        encoding="utf-8"
    ).lower()
    forbidden = [
        "executor.invoke",
