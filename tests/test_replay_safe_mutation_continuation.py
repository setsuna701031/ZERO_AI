...
def test_replay_and_recovery_snapshots_are_preserved() -> None:
    ...
    assert summary["verified_mutation_replay_snapshot"]["replay_id"] == "replay-verified"
    assert summary["verified_mutation_recovery_snapshot"]["recovery_id"] == "recovery-verified"
    assert summary["verified_mutation_chain"]["replay_id"] == "replay-verified"
    assert summary["verified_mutation_chain"]["continuation_cycle_id"] == "cycle-verified"


def test_missing_replay_snapshot_denies_reentry() -> None:
    from core.runtime.repair_transaction_execution_bridge import (
        build_verified_mutation_continuation_summary,
    )

    request = bridge_request()
    request["mutation_bridge_replay_snapshot"] = {}
    request["mutation_bridge_lineage"]["replay_continuity_summary"] = {}
    ...
    assert summary["verified_mutation_state"] == "constitutional_reentry_denied_missing_replay_snapshot"
    assert summary["reentry_replay_safe"] is False
