import ast
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
MODULE = ROOT / "core/runtime/runtime_activation_queue_storage_adapter.py"
DOC = ROOT / "docs/runtime_activation_queue_storage_adapter.md"
PACKAGE_SEQUENCE = ROOT / "docs/aer_evolution_v2_package_sequence.md"


def test_queue_storage_adapter_imports_and_public_api_exists():
    from core.runtime import runtime_activation_queue_storage_adapter

    assert hasattr(
        runtime_activation_queue_storage_adapter,
        "preview_runtime_activation_queue_storage_adapter",
    )
    assert runtime_activation_queue_storage_adapter.__all__ == [
        "preview_runtime_activation_queue_storage_adapter"
    ]


def test_queue_storage_adapter_preview_is_deterministic():
    from core.runtime.runtime_activation_queue_storage_adapter import (
        preview_runtime_activation_queue_storage_adapter,
    )

    factory = {
        "identity_snapshot": {"task_id": "task-1", "task_name": "Preview"},
        "lineage_snapshot": {"lineage_id": "lineage-1", "trace_id": "trace-1"},
        "queue_record_preview": {"record_kind": "runtime_queue_task"},
    }

    first = preview_runtime_activation_queue_storage_adapter(factory)
    second = preview_runtime_activation_queue_storage_adapter(factory)

    assert first == second
    assert first["mode"] == "queue_storage_adapter_preview"
    assert first["storage_status"] == "disabled"
    assert first["storage_reason"] == "queue_storage_adapter_disabled"
    assert first["result"] == "blocked"


def test_queue_storage_adapter_keeps_storage_and_mutation_disabled():
    from core.runtime.runtime_activation_queue_storage_adapter import (
        preview_runtime_activation_queue_storage_adapter,
    )

    result = preview_runtime_activation_queue_storage_adapter({})

    assert result["storage_adapter_ready"] is True
    assert result["storage_adapter_available"] is False
    assert result["storage_write_allowed"] is False
    assert result["queue_storage_mutated"] is False
    assert result["runtime_mutation_allowed"] is False
    assert result["filesystem_write_allowed"] is False
    assert result["database_write_allowed"] is False
    assert result["runtime_state_mutated"] is False
    assert result["repo_state_mutated"] is False


def test_queue_storage_adapter_has_no_downstream_dependency_flags():
    from core.runtime.runtime_activation_queue_storage_adapter import (
        preview_runtime_activation_queue_storage_adapter,
    )

    result = preview_runtime_activation_queue_storage_adapter({})

    assert result["scheduler_call_allowed"] is False
    assert result["executor_call_allowed"] is False
    assert result["tool_execution_allowed"] is False
    assert result["subprocess_allowed"] is False
    assert result["background_loop_started"] is False


def test_queue_storage_adapter_snapshots_metadata_and_validates_record_shape():
    from core.runtime.runtime_activation_queue_storage_adapter import (
        preview_runtime_activation_queue_storage_adapter,
    )

    factory = {
        "identity_snapshot": {"task_name": "Preview", "task_id": "task-1"},
        "lineage_snapshot": {"trace_id": "trace-1", "lineage_id": "lineage-1"},
        "queue_record_preview": {
            "record_status": "preview_only",
            "record_kind": "runtime_queue_task",
        },
        "extra": {"nested": True},
    }
    original = {
        "identity_snapshot": {"task_name": "Preview", "task_id": "task-1"},
        "lineage_snapshot": {"trace_id": "trace-1", "lineage_id": "lineage-1"},
        "queue_record_preview": {
            "record_status": "preview_only",
            "record_kind": "runtime_queue_task",
        },
        "extra": {"nested": True},
    }

    result = preview_runtime_activation_queue_storage_adapter(factory)

    assert factory == original
    assert result["identity_snapshot"] == {
        "task_id": "task-1",
        "task_name": "Preview",
    }
    assert result["lineage_snapshot"] == {
        "lineage_id": "lineage-1",
        "trace_id": "trace-1",
    }
    assert result["queue_record_preview"] == {
        "record_kind": "runtime_queue_task",
        "record_status": "preview_only",
    }
    assert result["storage_target_preview"] == {
        "target_type": "persistent_runtime_queue",
        "target_status": "future_package",
        "adapter_available": False,
        "write_enabled": False,
        "record_shape_valid": True,
    }


def test_queue_storage_adapter_handles_none_and_malformed_input():
    from core.runtime.runtime_activation_queue_storage_adapter import (
        preview_runtime_activation_queue_storage_adapter,
    )

    none_result = preview_runtime_activation_queue_storage_adapter(None)
    assert none_result["input_present"] is False
    assert none_result["input_type"] == "NoneType"
    assert none_result["input_keys"] == []
    assert none_result["storage_adapter_available"] is False

    for malformed in ("bad", 7, ["not", "mapping"], object()):
        result = preview_runtime_activation_queue_storage_adapter(malformed)
        assert result["storage_adapter_available"] is False
        assert result["storage_write_allowed"] is False
        assert result["queue_storage_mutated"] is False
        assert result["runtime_mutation_allowed"] is False


def test_ast_has_no_downstream_imports_or_forbidden_calls():
    source = MODULE.read_text(encoding="utf-8")
    tree = ast.parse(source)
    imported_modules = []
    forbidden_calls = {
        "open",
        "Path",
        "Popen",
        "run",
        "call",
        "check_call",
        "check_output",
        "connect",
        "execute",
    }

    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            imported_modules.extend(alias.name for alias in node.names)
        elif isinstance(node, ast.ImportFrom):
            imported_modules.append(node.module or "")
        elif isinstance(node, ast.Call):
            if isinstance(node.func, ast.Name):
                assert node.func.id not in forbidden_calls
            elif isinstance(node.func, ast.Attribute):
                assert node.func.attr not in {"write", "run", "call", "start", "execute"}

    assert not any("queue" in module.lower() for module in imported_modules)
    assert not any("storage" in module.lower() for module in imported_modules)
    assert not any("sqlite" in module.lower() for module in imported_modules)
    assert not any("scheduler" in module.lower() for module in imported_modules)
    assert not any("executor" in module.lower() for module in imported_modules)
    assert not any("subprocess" in module.lower() for module in imported_modules)
    assert not any("tool" in module.lower() for module in imported_modules)


def test_docs_explain_disabled_storage_adapter_boundary():
    assert DOC.exists()
    text = DOC.read_text(encoding="utf-8")

    for phrase in (
        "preview-only",
        "must not persist any queue data",
        "Filesystem writes are forbidden",
        "Database writes are forbidden",
        "Queue implementation imports are forbidden",
        "Scheduler imports and calls are forbidden",
        "Executor imports and calls are forbidden",
        "Tool execution is forbidden",
        "Queue storage mutation is forbidden",
        "GO only for disabled queue storage adapter preview",
    ):
        assert phrase in text


def test_package_sequence_records_889_896():
    text = PACKAGE_SEQUENCE.read_text(encoding="utf-8")
    assert "Packages 889-896" in text
    assert "Runtime Queue Storage Adapter Boundary (Disabled)" in text
