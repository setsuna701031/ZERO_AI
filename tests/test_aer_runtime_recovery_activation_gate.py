from __future__ import annotations

import inspect

from core.runtime import aer_runtime_recovery_activation_gate as gate


def _valid_invocation() -> dict[str, object]:
    return {
        "contract": gate.RECOVERY_BINDING_ENDPOINT_INVOCATION_REPORT_CONTRACT,
        "prepared": True,
        "blocked": False,
        "denied": False,
        "status": "prepared",
        "endpoint_name": gate.RECOVERY_BINDING_ENDPOINT_NAME,
        "endpoint_declared": True,
        "endpoint_enabled": False,
        "endpoint_invokable": False,
        "endpoint_invoked": False,
        "invocation_allowed": False,
        "binding_disabled": True,
        "binding_applied": False,
        "runtime_hook_registered": False,
        "event_emitted": False,
        "recovery_enabled": False,
        "executes_recovery": False,
        "side_effects_performed": False,
        "plain_dict_only": True,
    }


def test_public_surface_is_strict() -> None:
    assert gate.__all__ == [
        "RECOVERY_BINDING_ENDPOINT_INVOCATION_REPORT_CONTRACT",
        "RECOVERY_BINDING_ENDPOINT_NAME",
        "RECOVERY_ACTIVATION_GATE_REPORT_CONTRACT",
        "RECOVERY_ACTIVATION_GATE_ALLOWED_STATUSES",
        "RECOVERY_ACTIVATION_GATE_DENIED_CAPABILITIES",
        "prepare_recovery_activation_gate",
    ]


def test_prepared_activation_gate_remains_closed() -> None:
    report = gate.prepare_recovery_activation_gate(_valid_invocation(), gate_id="gate-1")

    assert report["contract"] == gate.RECOVERY_ACTIVATION_GATE_REPORT_CONTRACT
    assert report["prepared"] is True
    assert report["blocked"] is False
    assert report["denied"] is False
    assert report["status"] == "prepared"
    assert report["gate_enabled"] is False
    assert report["gate_open"] is False
    assert report["activation_allowed"] is False
    assert report["activation_gate_enabled"] is False
    assert report["activation_gate_opened"] is False
    assert report["kill_switch_required"] is True
    assert report["admission_required"] is True
    assert report["endpoint_invocation_required"] is True
    assert report["endpoint_invoked"] is False
    assert report["binding_disabled"] is True
    assert report["runtime_hook_registered"] is False
    assert report["event_emitted"] is False
    assert report["recovery_enabled"] is False
    assert report["single_entry_only"] is True
    assert report["executes_recovery"] is False
    assert report["side_effects_performed"] is False
    assert report["plain_dict_only"] is True


def test_invalid_invocation_blocks_gate() -> None:
    report = gate.prepare_recovery_activation_gate({"contract": "wrong"})
    assert report["prepared"] is False
    assert report["blocked"] is True
    assert report["denied"] is False
    assert report["status"] == "blocked"
    assert report["binding_endpoint_invocation_reference"] == {}
    assert "missing or incompatible" in str(report["reason"])


def test_gate_open_request_is_denied() -> None:
    report = gate.prepare_recovery_activation_gate(_valid_invocation(), request_gate_open=True)
    assert report["prepared"] is False
    assert report["blocked"] is False
    assert report["denied"] is True
    assert report["status"] == "denied"
    assert report["gate_open"] is False
    assert "prohibited" in str(report["reason"])


def test_blocked_and_unsupported_statuses_are_passive() -> None:
    blocked = gate.prepare_recovery_activation_gate(_valid_invocation(), requested_status="blocked")
    assert blocked["blocked"] is True
    assert blocked["gate_open"] is False

    unsupported = gate.prepare_recovery_activation_gate(_valid_invocation(), requested_status="unknown")
    assert unsupported["blocked"] is False
    assert unsupported["prepared"] is False
    assert unsupported["denied"] is False
    assert unsupported["status"] == "blocked"
    assert "unsupported" in str(unsupported["reason"])


def test_activation_gate_source_avoids_runtime_imports() -> None:
    source = inspect.getsource(gate)
    forbidden_imports = [
        "import os",
        "import pathlib",
        "import subprocess",
        "import scheduler",
        "import task_runner",
        "import dispatcher",
        "import supervisor",
        "import native",
    ]
    for token in forbidden_imports:
        assert token not in source
