...
def _canonical_request_callers(paths: list[Path]) -> list[Path]:
    forbidden = [
        "aer_runtime_recovery_canonical_request",
        "prepare_canonical_runtime_recovery_request",
    ]
    allowed = {
        "aer_runtime_recovery_canonical_request.py",
...
def test_only_package_251_integration_may_call_canonical_request() -> None:
    assert _canonical_request_callers(list(Path("core/runtime").glob("*.py"))) == []


def test_second_runtime_canonical_request_caller_is_rejected(tmp_path: Path) -> None:
    second = tmp_path / "second_runtime_caller.py"
    second.write_text("from core.runtime.aer_runtime_recovery_canonical_request import prepare_canonical_runtime_recovery_request\n", encoding="utf-8")
    assert _canonical_request_callers([second]) == [second]
