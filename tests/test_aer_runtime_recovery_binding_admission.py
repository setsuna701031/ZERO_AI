...
def test_binding_admission_evaluation_prepared_but_not_admitted():
    report = prepare_recovery_binding_admission_evaluation(_disabled_binding(), _binding_points())
    assert report["contract"] == RECOVERY_BINDING_ADMISSION_EVALUATION_CONTRACT
    assert report["prepared"] is True
    assert report["status"] == "prepared"
    assert report["admission_evaluated"] is True
    ...
    assert report["canonical_event"]["entry_id"] == RECOVERY_BINDING_ADMISSION_ENTRY
    assert report["admission_evaluation_only"] is True
...
def test_binding_admission_blocks_invalid_references():
    report = prepare_recovery_binding_admission_evaluation({}, _binding_points())
...
def test_binding_admission_denies_requested_admission_grant():
    report = prepare_recovery_binding_admission_evaluation(
        _disabled_binding(),
        _binding_points(),
    ...
    assert report["admission_allowed"] is False
    assert report["binding_admitted"] is False
    assert "binding_admission_grant" in report["denied_capabilities"]


def test_binding_admission_rejects_non_single_entry():
    report = prepare_recovery_binding_admission_evaluation(
        _disabled_binding(),
        _binding_points(),
        requested_entry="scheduler_entry",
    )
    assert report["prepared"] is False
    assert report["blocked"] is True
