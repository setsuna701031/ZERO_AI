...
def test_missing_verification_denies_constitutional_reentry() -> None:
    ...
    result.metadata["verification"] = {"status": "failed"}
    ...
    assert summary["verified_mutation_state"] == "constitutional_reentry_denied_missing_verification"
    assert summary["reentry_requires_review"] is True


def test_terminal_repair_block_denies_constitutional_reentry() -> None:
    from core.runtime.repair_transaction_execution_bridge import (
        build_verified_mutation_continuation_summary,
    )
    ...
    summary = build_verified_mutation_continuation_summary(
        successful_result(),
        bridge_request=bridge_request(terminal=True),
        transaction=transaction(),
    )

    assert summary["constitutional_reentry_allowed"] is False
    assert summary["verified_mutation_state"] == "constitutional_reentry_blocked_terminal"
    assert summary["reentry_terminality"] == "terminal"
