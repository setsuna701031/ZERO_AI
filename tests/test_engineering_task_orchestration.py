...
def ready(tmp_path):
    s=create_task(tmp_path, req()); tid=s['task_id']; admit_task(tmp_path,tid); attach_analysis(tmp_path,tid,art('analysis')); attach_candidate_selection(tmp_path,tid,art('candidate')); attach_plan(tmp_path,tid,art('plan')); attach_proposal(tmp_path,tid,art('proposal')); return tid
...
def test_lifecycle_wait_and_replay(tmp_path):
    ...
    approval=art('approval','approved', decision='approved')
    ...
    with pytest.raises(Exception): attach_human_approval(tmp_path,tid,art('approval','approved', decision='approved', fingerprint='other'))
...
def test_full_progression_with_monkeypatched_executor(tmp_path, monkeypatch):
    tid=ready(tmp_path); attach_human_approval(tmp_path,tid,art('approval','approved', decision='approved'))
    attach_authorization(tmp_path,tid,art('auth','verified', authorized_scope_id='scope-1', authorized_scope_fingerprint='sf'))
    attach_preparation(tmp_path,tid,art('prep','closed'))
    attach_authorization_token(tmp_path,tid,art('token','issued', token_consumed=False, use_limit=1, transaction_id='tx-1'))
    calls=[]
    def fake(handoff, workspace_root, execute_confirmed=False):
        calls.append(1); return {'result':{'schema':'zero.engineering.workspace_mutation_result.v1','result_id':'res-1','fingerprint':'resfp','status':'succeeded'},'execution_evidence':{'schema':'zero.engineering.workspace_mutation_execution_evidence.v1','evidence_id':'ev-1','fingerprint':'evfp'},'execution_closure':{'schema':'zero.engineering.workspace_mutation_execution_closure.v1','closure_id':'tc-1','fingerprint':'tcfp'}}
    monkeypatch.setattr('core.engineering.engineering_governed_workspace_mutation_executor.execute_pipeline', fake)
    s=execute_task(tmp_path,tid,{'handoff_id':'h'},tmp_path); assert s['lifecycle_state']=='executed' and calls==[1]
    with pytest.raises(Exception): execute_task(tmp_path,tid,{},tmp_path)
    v={'schema':'zero.engineering.task_verification.v1','verification_id':'ver-1','fingerprint':'vfp','status':'passed','task_id':tid,'transaction_identity':s['transaction_identity'],'performed_verification_set':['unit'],'failed_count':0}
