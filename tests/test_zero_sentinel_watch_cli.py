...
def test_watch_sentinel_runs_bounded_idle_cycles(tmp_path: Path) -> None:
    queue_path = tmp_path / "queue.json"
    _write_queue(queue_path, [])
    ...
    assert result["watch_status"] == "max_cycles_reached"
    assert result["cycle_count"] == 2
    assert result["idle_count"] == 2


def test_watch_sentinel_persists_existing_completed_queue(tmp_path: Path) -> None:
    queue_path = tmp_path / "queue.json"
    _write_queue(
        queue_path,
        [
