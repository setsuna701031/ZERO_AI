import ast
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
MODULE = ROOT / "core/runtime/runtime_activation_queue_persistence_preview.py"
DOC = ROOT / "docs/runtime_activation_queue_persistence_preview.md"
PACKAGE_SEQUENCE = ROOT / "docs/aer_evolution_v2_package_sequence.md"


def test_queue_persistence_preview_imports_and_public_api_exists():
    from core.runtime import runtime_activation_queue_persistence_preview

    assert hasattr(
        runtime_activation_queue_persistence_preview,
        "preview_runtime_activation_queue_persistence",
    )
    assert runtime_activation_queue_persistence_preview.__all__ == [
        "preview_runtime_activation_queue_persistence"
    ]


def test_queue_persistence_preview_is_deterministic():
    from core.runtime.runtime_activation_queue_persistence_preview import (
        preview_runtime_activation_queue_persistence,
    )

    commit_gate = {
        "identity_snapshot": {"task_id": "task-1", "task_name": "Preview"},
        "lineage_snapshot": {"lineage_id": "lineage-1", "trace_id": "trace-1"},
    }

    first = preview_runtime_activation_queue_persistence(commit_gate)
    second = preview_runtime_activation_queue_persistence(commit_gate)

    assert first == second
    assert first["mode"] == "queue_persistence_preview"
    assert first["persistence_status"] == "disabled"
    assert first["persistence_reason"] == "queue_persistence_disabled"
    assert first["result"] == "blocked"


def test_queue_persistence_write_and_mutation_are_disabled():
    from core.runtime.runtime_activation_queue_persistence_preview import (
        preview_runtime_activation_queue_persistence,
    )

    result = preview_runtime_activation_queue_persistence({})

    assert result["persistence_preview_ready"] is True
    assert result["queue_persistence_allowed"] is False
    assert result["queue_write_allowed"] is False
    assert result["runtime_mutation_allowed"] is False
    assert result["runtime_state_mutated"] is False
    assert result["repo_state_mutated"] is False


def test_queue_persistence_has_no_downstream_dependency_flags():
    from core.runtime.runtime_activation_queue_persistence_preview import (
        preview_runtime_activation_queue_persistence,
    )

    result = preview_runtime_activation_queue_persistence({})

    assert result["scheduler_call_allowed"] is False
    assert result["executor_call_allowed"] is False
    assert result["tool_execution_allowed"] is False
    assert result["subprocess_allowed"] is False
    assert result["background_worker_started"] is False


def test_queue_persistence_snapshots_identity_lineage_and_target_without_mutating_input():
    from core.runtime.runtime_activation_queue_persistence_preview import (
        preview_runtime_activation_queue_persistence,
    )

    commit_gate = {
        "identity_snapshot": {"task_name": "Preview", "task_id": "task-1"},
        "lineage_snapshot": {"trace_id": "trace-1", "lineage_id": "lineage-1"},
        "extra": {"nested": True},
    }
    original = {
        "identity_snapshot": {"task_name": "Preview", "task_id": "task-1"},
        "lineage_snapshot": {"trace_id": "trace-1", "lineage_id": "lineage-1"},
        "extra": {"nested": True},
    }

    result = preview_runtime_activation_queue_persistence(commit_gate)

    assert commit_gate == original
    assert result["input_present"] is True
    assert result["input_type"] == "dict"
    assert result["input_keys"] == ["extra", "identity_snapshot", "lineage_snapshot"]
    assert result["identity_snapshot"] == {
        "task_id": "task-1",
        "task_name": "Preview",
    }
    assert result["lineage_snapshot"] == {
        "lineage_id": "lineage-1",
        "trace_id": "trace-1",
    }
    assert result["future_persistence_target"] == {
        "target_type": "runtime_queue",
        "target_status": "future_package",
        "write_enabled": False,
    }


def test_queue_persistence_handles_none_and_malformed_input():
    from core.runtime.runtime_activation_queue_persistence_preview import (
        preview_runtime_activation_queue_persistence,
    )

    none_result = preview_runtime_activation_queue_persistence(None)
    assert none_result["input_present"] is False
    assert none_result["input_type"] == "NoneType"
    assert none_result["input_keys"] == []
    assert none_result["queue_persistence_allowed"] is False

    for malformed in ("bad", 7, ["not", "mapping"], object()):
        result = preview_runtime_activation_queue_persistence(malformed)
        assert result["queue_persistence_allowed"] is False
        assert result["queue_write_allowed"] is False
        assert result["runtime_mutation_allowed"] is False


def test_ast_has_no_downstream_imports_or_forbidden_calls():
    source = MODULE.read_text(encoding="utf-8")
    tree = ast.parse(source)
    imported_modules = []
    forbidden_calls = {
        "open",
        "Path",
        "Popen",
        "run",
        "call",
        "check_call",
        "check_output",
    }

    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            imported_modules.extend(alias.name for alias in node.names)
        elif isinstance(node, ast.ImportFrom):
            imported_modules.append(node.module or "")
        elif isinstance(node, ast.Call):
            if isinstance(node.func, ast.Name):
                assert node.func.id not in forbidden_calls
            elif isinstance(node.func, ast.Attribute):
                assert node.func.attr not in {"write", "run", "call", "start"}

    assert not any("queue" in module.lower() for module in imported_modules)
    assert not any("scheduler" in module.lower() for module in imported_modules)
    assert not any("executor" in module.lower() for module in imported_modules)
    assert not any("subprocess" in module.lower() for module in imported_modules)
    assert not any("tool" in module.lower() for module in imported_modules)


def test_docs_explain_disabled_persistence_preview_boundary():
    assert DOC.exists()
    text = DOC.read_text(encoding="utf-8")

    for phrase in (
        "preview-only persistence boundary",
        "Future queue persistence remains unimplemented",
        "Queue writes are forbidden",
        "File IO is forbidden",
        "Queue implementation imports are forbidden",
        "Scheduler imports and calls are forbidden",
        "Executor imports and calls are forbidden",
        "Subprocess use is forbidden",
        "Tool execution is forbidden",
        "Runtime mutation is forbidden",
        "GO only for disabled queue persistence preview",
    ):
        assert phrase in text


def test_package_sequence_records_865_872():
    text = PACKAGE_SEQUENCE.read_text(encoding="utf-8")
    assert "Packages 865-872" in text
    assert "Runtime Queue Persistence Preview Boundary (Disabled)" in text
