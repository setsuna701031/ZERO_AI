...
@dataclass(frozen=True)
class RuntimeReplayRevalidationRecord:
    revalidation_id: str
    original_execution_id: str
    replay_execution_id: str
    original_outcome_fingerprint: str
    replay_outcome_fingerprint: str
    original_result_hash: str
    replay_result_hash: str
    outcome_match: bool
    result_match: bool
...
class RuntimeReplaySession:
    ...
    revalidation_records: list[RuntimeReplayRevalidationRecord] = field(default_factory=list)
...
class RuntimeReplayEngine:
    ...
    def attach_integrity_record(
        ...
        updated = replace(
            ...
            review_required=(
                ...
                or constitution["review_required"]
                or (not integrity_record.integrity_verified)
            ),
            block_recommended=(
                transition["block_recommended"]
                or constitution["block_recommended"]
                or (not integrity_record.integrity_verified)
            ),
    ...
    def record_execution_outcome_revalidation(
        self,
        *,
        original_execution_id: str,
        replay_execution_id: str,
        ...
        replay_result: Any,
        metadata: dict[str, Any] | None = None,
    ) -> RuntimeReplayRevalidationRecord:
        original_payload = _plain_result(original_result)
        replay_payload = _plain_result(replay_result)
        original_result_hash = self._hash_result(original_payload)
        replay_result_hash = self._hash_result(replay_payload)

        original_outcome_fingerprint = _extract_outcome_fingerprint(original_payload)
        replay_outcome_fingerprint = _extract_outcome_fingerprint(replay_payload)

        if original_outcome_fingerprint and replay_outcome_fingerprint:
            outcome_match = original_outcome_fingerprint == replay_outcome_fingerprint
        else:
            outcome_match = original_result_hash == replay_result_hash

        result_match = original_result_hash == replay_result_hash
        legality_verified = (
            _extract_execution_legality(original_payload)
            and _extract_execution_legality(replay_payload)
            and not _extract_runtime_frozen(original_payload)
            and not _extract_runtime_frozen(replay_payload)
        )
        deterministic = bool(outcome_match and legality_verified)
        runtime_frozen = not deterministic
        reason = (
            "deterministic_replay_revalidation_verified"
            if deterministic
            else "deterministic_replay_revalidation_mismatch"
        )

        proof_payload = {
            "original_execution_id": str(original_execution_id),
            "replay_execution_id": str(replay_execution_id),
            "original_outcome_fingerprint": original_outcome_fingerprint,
            "replay_outcome_fingerprint": replay_outcome_fingerprint,
        ...
        proof_fingerprint = _hash_payload(proof_payload)
        return RuntimeReplayRevalidationRecord(
            revalidation_id=f"runtime_replay_revalidation:{proof_fingerprint[:16]}",
            original_execution_id=str(original_execution_id),
            replay_execution_id=str(replay_execution_id),
            original_outcome_fingerprint=original_outcome_fingerprint,
            replay_outcome_fingerprint=replay_outcome_fingerprint,
    ...
    def attach_revalidation_record(
        self,
        replay_id: str,
        revalidation_record: RuntimeReplayRevalidationRecord,
    ) -> RuntimeReplaySession:
        replay = self._replays.get(replay_id)
        if replay is None:
            raise RuntimeReplayRejected(
        ...
        transition = runtime_status_transition_payload(
            replay.canonical_status,
            status_from_replay_state(
                "replayed" if replay.verified and revalidation_record.deterministic else "failed"
            ),
            source="runtime_replay_engine",
        )
        metadata = replay.metadata if isinstance(replay.metadata, dict) else {}
        revalidation_summary = {
            "revalidation_id": revalidation_record.revalidation_id,
            "deterministic": revalidation_record.deterministic,
            "runtime_frozen": revalidation_record.runtime_frozen,
            "reason": revalidation_record.reason,
            "proof_fingerprint": revalidation_record.proof_fingerprint,
            "outcome_match": revalidation_record.outcome_match,
            "result_match": revalidation_record.result_match,
            "legality_verified": revalidation_record.legality_verified,
        }
        constitution = replay_constitution_summary(
            replay_id=replay.replay_id,
            parent_replay_lineage=replay.parent_replay_lineage,
            ...
            metadata={
                **metadata,
                "deterministic_replay_revalidation": revalidation_summary,
                "replay_lineage_corrupted": bool(revalidation_record.runtime_frozen),
            },
        )
        updated = replace(
            replay,
            revalidation_records=[
                *replay.revalidation_records,
                revalidation_record,
            ],
            verified=replay.verified and revalidation_record.deterministic,
            canonical_status=transition["to_status"],
            transition_allowed=transition["allowed"],
            transition_regression=transition["regression"],
            transition_reason=transition["transition_reason"],
            ...
            block_recommended=transition["block_recommended"] or constitution["block_recommended"] or revalidation_record.runtime_frozen,
            constitutional_continuity=constitution["constitutional_continuity"],
            continuity_verified=constitution["continuity_verified"] and revalidation_record.deterministic,
            continuity_break=constitution["continuity_break"] or ("" if revalidation_record.deterministic else revalidation_record.reason),
            replay_constitution_status=constitution["replay_constitution_status"],
            enforcement_visibility=constitution["enforcement_visibility"],
            enforcement_snapshot={
                **constitution["enforcement_snapshot"],
                "deterministic_replay_revalidation": revalidation_summary,
            },
        )
        self._replays[replay_id] = updated
    ...
    def _copy_replay(self, replay: RuntimeReplaySession) -> RuntimeReplaySession:
        return replace(
            ...
            revalidation_records=list(replay.revalidation_records),
...
def _plain_result(result: Any) -> Any:
    if hasattr(result, "to_dict"):
        try:
            ...
            pass
    if dataclass_is_instance(result):
        try:
...
def _extract_result_mapping(result: Any) -> dict[str, Any]:
    plain = _plain_result(result)
    return plain if isinstance(plain, dict) else {}


def _extract_metadata(result: Any) -> dict[str, Any]:
    mapping = _extract_result_mapping(result)
    metadata = mapping.get("metadata")
    return copy.deepcopy(metadata) if isinstance(metadata, dict) else {}


def _extract_outcome_fingerprint(result: Any) -> str:
    mapping = _extract_result_mapping(result)
    metadata = _extract_metadata(mapping)

    for source in (mapping, metadata):
        value = source.get("execution_outcome_fingerprint") if isinstance(source, dict) else None
        if value:
            return str(value)

    outcome = mapping.get("outcome_closure")
    if isinstance(outcome, dict) and outcome.get("outcome_fingerprint"):
        return str(outcome.get("outcome_fingerprint"))

    outcome = metadata.get("execution_outcome_closure")
    if isinstance(outcome, dict) and outcome.get("outcome_fingerprint"):
        return str(outcome.get("outcome_fingerprint"))
...
def _extract_execution_legality(result: Any) -> bool:
    mapping = _extract_result_mapping(result)
    metadata = _extract_metadata(mapping)
    for source in (mapping, metadata):
        if not isinstance(source, dict):
            continue
        if source.get("execution_legality") not in (None, "", "legal"):
            return False
        verification = source.get("execution_outcome_verification")
        if isinstance(verification, dict):
            if verification.get("verification_state") not in (None, "", "verified"):
                return False
            if verification.get("legality_verified") is False:
                return False
    return True


def _extract_runtime_frozen(result: Any) -> bool:
    mapping = _extract_result_mapping(result)
    metadata = _extract_metadata(mapping)
    for source in (mapping, metadata):
        if isinstance(source, dict) and bool(source.get("runtime_frozen")):
            return True
    outcome = mapping.get("outcome_closure")
    if isinstance(outcome, dict) and bool(outcome.get("frozen")):
        return True
    outcome = metadata.get("execution_outcome_closure")
    if isinstance(outcome, dict) and bool(outcome.get("frozen")):
        return True
...
def build_deterministic_replay_revalidation_record(
    *,
    original_execution_id: str,
    replay_execution_id: str,
    original_result: Any,
    ...
    return engine.record_execution_outcome_revalidation(
...
def build_sovereign_archive_replay_validation(
    *,
    workflow_runtime_session: dict[str, Any],
) -> dict[str, Any]:
    """Validate sovereign archive / constitutional resurrection replay continuity.

    This helper is intentionally read-only.  It delegates continuity authority to
    ...
    try:
        ...
        return {
            "ok": False,
            "schema": "zero.runtime_replay.sovereign_archive_validation.v1",
            "error": str(exc),
        }
    return {
        "ok": bool(summary.get("ok", False)),
...
def build_governance_storage_lifecycle_replay_validation(
    *,
    workflow_runtime_session: dict[str, Any],
) -> dict[str, Any]:
    """Validate runtime governance query / storage lifecycle replay continuity.

    This helper is read-only. It delegates continuity authority to
    WorkflowRuntimeSessionManager and does not execute, mutate, approve, or
    arbitrate anything.
    """
    try:
        ...
        return {
            "ok": False,
            "schema": "zero.runtime_replay.governance_storage_lifecycle_validation.v1",
            "error": str(exc),
        }
    return {
        "ok": bool(summary.get("ok", False)),
...
__all__ = [
    "REPLAY_CONSTITUTION_CANONICAL",
    "REPLAY_CONSTITUTION_REVIEW_REQUIRED",
    "REPLAY_CONSTITUTION_BLOCK_RECOMMENDED",
    "RuntimeReplayRecord",
    "RuntimeReplayIntegrityRecord",
    "RuntimeReplayRevalidationRecord",
    "RuntimeReplaySession",
    ...
    "build_deterministic_replay_revalidation_record",
    "build_replayable_workflow_runtime_session",
    "workflow_replay_graph_summary",
    "build_constitutional_self_amendment_replay_validation",
    "build_epoch_migration_replay_validation",
    "build_sovereign_archive_replay_validation",
    "build_governance_kernel_consolidation_replay_validation",
    "build_governance_storage_lifecycle_replay_validation",
]
