"""Disabled Runtime Recovery activation simulation decision reports."""
...
RECOVERY_ACTIVATION_SIMULATION_DECISION_ALLOWED_STATUSES = ("prepared", "blocked", "denied")
...
def prepare_recovery_activation_simulation_report(
    activation_simulation: Mapping[str, Any],
    *,
    report_id: str | None = None,
    requested_status: str = "prepared",
    request_simulation_commit: bool = False,
    metadata: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Prepare a deterministic simulation report without committing activation."""

    simulation = _plain_mapping(activation_simulation)
    valid = _valid_activation_simulation(simulation)
    denied = request_simulation_commit or requested_status == "denied"
    ...
    return {
        "contract": RECOVERY_ACTIVATION_SIMULATION_DECISION_REPORT_CONTRACT,
        "report_id": report_id,
        "prepared": prepared,
        "blocked": blocked,
        "denied": denied,
        "status": status,
        "simulation_report_prepared": prepared,
        "activation_simulation_reference": simulation if valid else {},
        ...
        "denied_capabilities": list(RECOVERY_ACTIVATION_SIMULATION_DECISION_DENIED_CAPABILITIES),
        "reason": _reason(requested_status, valid, request_simulation_commit),
        "metadata": _plain_mapping(metadata),
        "activation_simulation_report_only": True,
...
def _valid_activation_simulation(value: Mapping[str, Any]) -> bool:
    return (
        value.get("contract") == RECOVERY_ACTIVATION_SIMULATION_REPORT_CONTRACT
        and value.get("prepared") is True
        and value.get("blocked") is False
        and value.get("denied") is False
        and value.get("status") == "prepared"
        and value.get("simulation_declared") is True
        and value.get("simulation_only") is True
        and value.get("simulation_applied") is False
        and value.get("simulation_result") == "not_applied"
        ...
        and value.get("activation_simulation_only") is True
...
def _reason(requested_status: str, valid: bool, request_simulation_commit: bool) -> str | None:
    if request_simulation_commit:
        return "activation simulation commit is prohibited while Runtime Recovery remains disabled"
    if requested_status == "denied":
        return "caller requested passive denied activation simulation report status"
    if not valid:
        return "missing or incompatible disabled Recovery activation simulation"
    if requested_status == "blocked":
        return "caller requested passive blocked activation simulation report status"
    if requested_status != "prepared":
        return f"unsupported passive activation simulation report status: {requested_status}"
    return None
