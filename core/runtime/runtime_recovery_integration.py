...
def _resolve_final_status(
    ...
    if rollback_required and not rollback_executed and not approved:
...
def seal_runtime_recovery_integration(
    *,
    ...
    source_failure = {}
    for payload in (chain_payload, execution_payload, continuation_payload, meta):
        value = payload.get("source_failure") if isinstance(payload, dict) else None
        if isinstance(value, dict):
            source_failure = copy.deepcopy(value)
            break

    rollback_required = any(
        bool(payload.get("rollback_required"))
        for payload in (chain_payload, execution_payload, continuation_payload)
        if isinstance(payload, dict)
    )
    rollback_executed = any(
        bool(payload.get("rollback_executed"))
        for payload in (chain_payload, execution_payload, continuation_payload)
        if isinstance(payload, dict)
    )
    ...
    chain_status = _text(chain_payload.get("status"), chain_payload.get("final_status"), default="unknown")
    execution_status = _text(execution_payload.get("status"), execution_payload.get("final_status"), default="unknown")
    continuation_status = _text(
