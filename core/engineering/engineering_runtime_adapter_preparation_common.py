...
def contains_prohibited(v:Any)->bool:
    if isinstance(v,Mapping):
        safe_false={'executable','adapter_invoked','runtime_invoked','executor_invoked','scheduler_invoked','authority_consumed','mutation_performed','adapter_loaded'}
        return any((str(k).lower() in PROHIBITED_KEYS and not (str(k).lower() in safe_false and x is False)) or contains_prohibited(x) for k,x in v.items())
...
def operation_valid(v:Any)->bool: return isinstance(v,Mapping) and bool(v) and not contains_prohibited(v) and canonical_nonempty(v.get('operation_type'))
...
def authority_valid(a:Any, scope:Any)->bool:
    ...
    required={'non_transferable':True,'non_reusable':True,'scope_bound':True,'perpetual':False,'passive':True,'consumed':False,'closed':False,'unrestricted':False,'restricted':True}
    if any(a.get(k) is not val for k,val in required.items()): return False
    if a.get('scope') is None or a.get('scope')!=scope or not scope_bounded(a.get('scope'),scope): return False
    return not contains_wildcard(a)
def normalize_reasons(rs:Any)->list[str]: return sorted({r for r in rs if isinstance(r,str) and canonical_nonempty(r)})
def boundary()->dict[str,bool]: return {'sealed':True,'passive_only':True,'adapter_loaded':False,'adapter_invoked':False,'runtime_invoked':False,'executor_invoked':False,'scheduler_invoked':False,'authority_consumed':False,'mutation_performed':False}
def validate_artifact(value:Any,*,schema:str,statuses:set[str],id_key:str,prefix:str,fields:set[str])->ValidationResult:
    ...
    req={'schema',id_key,'fingerprint',*fields}; errors=[f'missing:{k}' for k in sorted(req-set(value))]+[f'unexpected:{k}' for k in sorted(set(value)-req)]
    if value.get('schema')!=schema: errors.append('invalid_contract')
    if statuses and not any(value.get(k) in statuses for k in ('preparation_status','eligibility_status','package_status')): errors.append('invalid_contract')
