...
PROHIBITED_KEYS=frozenset({'command','command_line','shell','shell_command','script','source_code','bytecode','executable','binary','patch','diff','process','subprocess','callable','callback','function','module_path','import_path','entrypoint','raw_arguments','activation_callback','activation_command','invocation_command','runtime_entrypoint','token','token_value','activation_token','bearer_token','credentials','password','private_key','secret_key','bearer','access_token','refresh_token','api_key','authorization_header','environment_secrets','token_material'})
PROHIBITED_STRINGS=('bearer ','-----begin','private key','password=','secret=','api_key=','access_token=','refresh_token=','authorization:','token=','#!/','&&',';','|','`','$(','def ','lambda ','import ')
WILDCARDS=frozenset({'*','all','global','unrestricted','any','everything','repository','repository-wide','system','system-wide','unbounded'})
FALSE_INVARIANTS=('activation_token_issued','token_material_present','adapter_loaded','adapter_activated','adapter_invoked','runtime_invoked','executor_invoked','scheduler_invoked','authority_consumed','mutation_performed')
...
def contains_prohibited(v:Any)->bool:
 if isinstance(v,Mapping):
  for k,x in v.items():
   key=str(k).lower()
   if key in PROHIBITED_KEYS and not (key=='token_request_id' or key.endswith('_reference') or (key=='executable' and x is False)): return True
...
def authorization_constraints_valid(v:Any)->bool: return passive_mapping(v) and v.get('passive') is True and v.get('deterministic') is True and v.get('executable',False) is False
...
def passive_invariants_valid(v:Any,authorized_ok:bool=False)->bool: return isinstance(v,Mapping) and v.get('passive_only',True) is True and (authorized_ok or not v.get('activation_authorized')) and not any(v.get(k) for k in FALSE_INVARIANTS)
...
def validate_artifact(value:Any,*,schema:str,id_key:str,prefix:str,fields:set[str],status_key:str|None=None,statuses:set[str]|None=None)->ValidationResult:
 ...
 if contains_prohibited(value): e.append('executable_payload'); e.append('credential_like_payload')
