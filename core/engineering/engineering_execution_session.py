...
EMPTY = None
STAGES = ("awaiting_approval","approved","authorized","prepared","ready_for_execution","awaiting_verification","verification_prepared","verification_admitted","verifying","awaiting_verification_result","verified","completion_eligible","completed","closed","blocked","failed")
...
def derive_current_stage(session: Mapping[str, Any]) -> str:
    ...
    if session.get("runtime_continuation_identity") and session.get("verification_result_identity"): return "completion_eligible" if session.get("bounded_summary",{}).get("completion_eligible") is True else ("verified" if session.get("bounded_summary",{}).get("verification_passed") is True else "verification_failed")
    if session.get("verification_result_identity"): return "verified"
    if session.get("verification_run_identity"): return "awaiting_verification_result"
    if session.get("verification_admission_identity"): return "verification_admitted"
    if session.get("verification_plan_identity"): return "verification_prepared"
...
def _history_for(session: Mapping[str, Any]) -> list[dict[str, Any]]:
    rows=[]
    schema_by={"proposal":"zero.engineering.change_proposal.v1","proposal_linkage":"zero.engineering.proposal_linkage.v1","approval":"zero.engineering.approval_decision.v1","authorization":"zero.engineering.authorization_decision.v1","authorized_scope":"zero.engineering.mutation_authorized_scope.v1","preparation":"zero.engineering.mutation_preparation_closure.v1","token":"zero.engineering.mutation_preparation_token.v1","execution":"zero.engineering.workspace_mutation_result.v1","verification_plan":"zero.engineering.verification_plan.v1","verification_admission":"zero.engineering.verification_admission.v1","verification_run":"zero.engineering.verification_run.v1","verification_result":"zero.engineering.verification_result.v1","runtime_continuation":"zero.engineering.runtime_continuation.v1","completion":"zero.engineering.completion.v1","closure":"zero.engineering.task_closure.v1"}
    stage_by={"proposal":"awaiting_approval","approval":"approved","authorization":"authorized","authorized_scope":"authorized","preparation":"prepared","token":"ready_for_execution","execution":"awaiting_verification","verification_plan":"verification_prepared","verification_admission":"verification_admitted","verification_run":"awaiting_verification_result","verification_result":"verified","runtime_continuation":"completion_eligible","completion":"completed","closure":"closed"}
    for name in ("proposal","approval","authorization","authorized_scope","preparation","token","execution","verification_plan","verification_admission","verification_run","verification_result","runtime_continuation","completion","closure"):
        ik,fk=REFS[name]
