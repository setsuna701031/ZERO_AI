...
STORE_FILES={'intake':'work-entry/natural-language-intake.json','finalized_intake':'work-entry/finalized-natural-language-intake.json','evidence':'work-entry/intake-repository-evidence.json','candidate':'work-entry/specification-candidate.json','clarification':'work-entry/specification-clarification.json','response':'work-entry/specification-clarification-response.json','confirmation':'work-entry/specification-confirmation.json'}
...
def build_finalized_intake(intake:Mapping[str,Any], candidate:Mapping[str,Any], confirmation:Mapping[str,Any], *, session_id:str)->dict[str,Any]:
    if confirmation.get('confirmation_status')!='confirmed': raise NaturalLanguageIntakeError('pre_finalization_intake_reference')
    candidate_ref=confirmation.get('specification_candidate_reference') or {}
    if candidate_ref.get('artifact_identity')!=candidate.get('specification_candidate_id') or candidate_ref.get('artifact_fingerprint')!=candidate.get('specification_candidate_fingerprint'): raise NaturalLanguageIntakeError('wrong_specification_lineage')
    if intake.get('intake_id')!=session_id: raise NaturalLanguageIntakeError('intake_session_mismatch')
    body={'schema':FINALIZED_INTAKE_SCHEMA,'session_id':session_id,'originating_intake_reference':_ref(intake,'intake_fingerprint'),'specification_candidate_reference':_ref(candidate,'specification_candidate_fingerprint'),'specification_confirmation_reference':_ref(confirmation,'confirmation_fingerprint'),'normalized_statement':intake.get('normalized_statement'),'intent_classification':intake.get('intent_classification'),'requested_mode':intake.get('requested_mode'),'repository_reference':intake.get('repository_reference'),'finalized_scope':list(confirmation.get('confirmed_scope') or []),'finalized_acceptance_criteria':list(confirmation.get('confirmed_acceptance_criteria') or []),'finalized_constraints':list(confirmation.get('confirmed_constraints') or []),'finalization_status':'finalized'}
    return _stable(body,'finalized_intake_fingerprint','finalized_intake_id','engineering-finalized-intake-')

def persist_finalized_intake(store_root:str|Path, session_id:str, intake:Mapping[str,Any], candidate:Mapping[str,Any], confirmation:Mapping[str,Any])->dict[str,Any]:
    finalized=build_finalized_intake(intake,candidate,confirmation,session_id=session_id)
    write_session_artifact(store_root,session_id,STORE_FILES['finalized_intake'],finalized)
    try: persisted=read_session_artifact(store_root,session_id,STORE_FILES['finalized_intake'])
    except (OSError,ValueError,KeyError) as exc: raise NaturalLanguageIntakeError('missing_finalized_intake') from exc
    if persisted!=finalized: raise NaturalLanguageIntakeError('wrong_intake_fingerprint')
    expected=_stable({k:v for k,v in persisted.items() if k not in {'finalized_intake_fingerprint','finalized_intake_id'}},'finalized_intake_fingerprint','finalized_intake_id','engineering-finalized-intake-')
    if expected!=persisted: raise NaturalLanguageIntakeError('wrong_intake_fingerprint')
    return persisted

def create_formal_work_request_from_confirmed_specification(candidate:Mapping[str,Any], confirmation:Mapping[str,Any], *, repository_root_reference='.', repository_identity:Mapping[str,Any]|None=None, finalized_intake:Mapping[str,Any]|None=None)->dict[str,Any]:
    ...
    if finalized_intake is not None:
        expected=_stable({k:v for k,v in finalized_intake.items() if k not in {'finalized_intake_fingerprint','finalized_intake_id'}},'finalized_intake_fingerprint','finalized_intake_id','engineering-finalized-intake-')
        if expected!=finalized_intake: raise NaturalLanguageIntakeError('wrong_intake_fingerprint')
        if finalized_intake.get('finalization_status')!='finalized': raise NaturalLanguageIntakeError('pre_finalization_intake_reference')
        if finalized_intake.get('specification_confirmation_reference')!=_ref(confirmation,'confirmation_fingerprint'): raise NaturalLanguageIntakeError('wrong_specification_lineage')
    lineage={**_ref(finalized_intake,'finalized_intake_fingerprint'),'finalization_status':'finalized'} if finalized_intake is not None else candidate.get('intake_reference')
    req=create_engineering_work_request(request_statement=candidate['task_title'],repository_identity=repository_identity or {'repository_id':'default'},repository_root_reference=repository_root_reference,requested_scope=scope or ['docs/status.txt'],constraints=list(confirmation.get('confirmed_constraints',[]))+['lineage:'+confirmation['confirmation_id']],acceptance_intent=acc,risk_classification=candidate.get('risk_hints',[{}])[0].get('risk_level','unknown'),requested_mode=candidate.get('requested_mode','governed_delivery'),source_actor_reference={'specification_decision_reference':_ref(confirmation,'confirmation_fingerprint'),'natural_language_lineage':lineage})
...
def inspect_natural_language_intake(store_root:str|Path='.zero-engineering-sessions', session_id:str|None=None)->dict[str,Any]:
    ...
    i=b.get(STORE_FILES['intake'],{}); fi=b.get(STORE_FILES['finalized_intake'],{}); c=b.get(STORE_FILES['candidate'],{}); cl=b.get(STORE_FILES['clarification'],{}); cf=b.get(STORE_FILES['confirmation'],{}); f=b.get('work-entry/request.json')
    ...
    lineage=((f or {}).get('source_actor_reference') or {}).get('natural_language_lineage') or {}; resolved=bool(fi) and lineage.get('artifact_identity')==fi.get('finalized_intake_id'); fp_match=resolved and lineage.get('artifact_fingerprint')==fi.get('finalized_intake_fingerprint'); session_match=resolved and lineage.get('session_id')==fi.get('session_id')
    missing=None if (not f or (resolved and fp_match and session_match)) else 'unresolved_intake_reference'
    return {'natural_language_intake_status':i.get('intake_status','not_initialized'),'original_statement':i.get('original_statement'),'normalized_statement':i.get('normalized_statement'),'intent_classification':i.get('intent_classification'),'repository_evidence_status':'collected' if b.get(STORE_FILES['evidence']) else 'not_started','specification_candidate_status':c.get('candidate_status','not_started'),'specification_confirmation_status':cf.get('confirmation_status','not_started'),'clarification_status':cl.get('clarification_status','not_started'),'blocking_question_count':len(cl.get('required_questions',[])),'confirmation_status':cf.get('confirmation_status','not_started'),'finalized_intake_status':fi.get('finalization_status','missing'),'finalized_intake_identity':fi.get('finalized_intake_id'),'finalized_intake_fingerprint':fi.get('finalized_intake_fingerprint'),'persisted_intake_path':STORE_FILES['finalized_intake'] if fi else None,'formal_work_request_status':'created' if f else 'not_started','work_request_id':(f or {}).get('work_request_id'),'work_request_fingerprint':(f or {}).get('work_request_fingerprint'),'work_request_intake_reference_status':'resolved' if resolved else 'missing' if not lineage else 'unresolved','lineage_resolvable':resolved,'lineage_fingerprint_match':fp_match,'lineage_session_match':session_match,'repository_analysis_reference_status':'available' if b.get(STORE_FILES['evidence']) else 'missing','missing_linkage_reason':missing if f else ('requires_specification_confirmation' if cf.get('confirmation_status')!='confirmed' else 'missing_work_request'),'next_governed_action':i.get('next_governed_action'),'timeline':[{'stage':x,'status':'Completed' if x in tl[:3] else 'Pending' if x=='Human Clarification' and cl.get('clarification_status')=='required' else 'Not Started'} for x in tl]}
