...
def build_workspace_execution_result(submission, preflight, controlled):
 cs=controlled.get('controlled_execution_status'); status={'completed':'succeeded','cancelled':'cancelled','rejected':'rejected','failed':'failed','invalid':'invalid'}.get(cs,'invalid')
 body={'schema':SCHEMA,'result_status':status,'submission_id':submission.get('submission_id'),'preflight_id':preflight.get('preflight_id'),'controlled_execution_id':controlled.get('controlled_execution_id'),'workspace_id':submission.get('workspace_id'),'admission_id':submission.get('admission_id'),'read_scope_id':submission.get('read_scope_id'),'operation':submission.get('operation'),'relative_path_fingerprint':controlled.get('relative_path_fingerprint'),'filesystem_mutation_performed':False,'network_access_performed':False,'subprocess_created':False,'runtime_kernel_invoked':False,'scheduler_invoked':False,'planner_invoked':False,'worker_invoked':False,'mission_invoked':False}
 if 'output' in controlled: body['output']=controlled['output']
 if 'failure' in controlled: body['failure']=controlled['failure']
 return canon(body,'result_id','res')
