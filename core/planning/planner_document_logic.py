...
def build_requirement_pack_steps(source_path: str) -> List[Dict[str, Any]]:
    summary_prompt = (
        ...
        "Your summary must preserve concrete implementation targets from the requirement.\n"
        "Do not replace exact filenames, exact utility names, or exact result files with vague wording.\n"
        "If exact filenames appear in the requirement, copy them exactly.\n\n"
        "Required sections:\n"
        "1. Project Goal\n"
        "2. Key Requirements\n"
        "3. Constraints\n"
        ...
        "- Keep exact filenames exactly as written when present.\n"
        "- If the requirement includes a Python utility and a result file, mention them explicitly.\n"
        "- If the requirement includes concrete verification targets such as sum, average, max, min, mention them explicitly.\n"
        "- Do not use JSON.\n"
    ...
    implementation_prompt = (
        ...
        "- You must preserve exact filenames exactly as written when present in the requirement.\n"
        "- Do not replace exact filenames with generic phrases such as 'Python function', 'script', 'utility', or 'result file'.\n"
        "- In Exact File Targets, explicitly list `number_stats.py` and `stats_result.txt` when they appear in the requirement.\n"
        "- In Implementation Steps, explicitly write a step that creates `number_stats.py`.\n"
        "- In Implementation Steps, explicitly write a step that writes output to `stats_result.txt`.\n"
        "- In Recommended Execution Order, keep `number_stats.py` before `stats_result.txt` generation.\n"
        "- In Verification Focus, explicitly mention checking sum, average, max, and min.\n"
        "- The literal strings `number_stats.py` and `stats_result.txt` must both appear somewhere in the plan when they appear in the requirement.\n"
        "- Do not use JSON.\n"
    ...
    checklist_prompt = (
        ...
        "- Keep exact filenames exactly as written when present.\n"
        "- If the requirement includes a Python utility, list that exact utility filename in Deliverable.\n"
        "- If the requirement includes a result file, list that exact result filename in Deliverable.\n"
        "- If the requirement includes verification fields such as sum, average, max, min, include them explicitly in Verification and Acceptance Criteria.\n"
        "- Do not replace exact filenames with vague wording.\n"
        "- Keep it plain text.\n"
        "- Do not use JSON.\n"
