...
class SimpleStepRunner:
    """
    Scheduler simple step execution helper.
    """
    ...
    def _execute_simple_step(
        ...
        guard_result = self.execution_guard.check_step(
            step=guard_step,
            task_dir=task_dir,
        )
        if not bool(guard_result.get("ok")):
            raise PermissionError(
                str(guard_result.get("error") or "guard blocked execution")
            )
        ...
        if step_type == "ensure_file":
            ...
            if not full_path:
                full_path = self._resolve_step_path(
                    raw_path,
                    task_dir=task_dir,
        ...
        if step_type == "command":
            ...
            if not gateway_result["ok"]:
                raise RuntimeError(
                    ...
                    f"returncode={gateway_result['returncode']} | "
        ...
        if step_type == "run_python":
            ...
            if not bool(read_guard.get("ok")):
                raise PermissionError(
                    str(read_guard.get("error") or "guard blocked python file read")
                )
        ...
        if step_type == "read_file":
            ...
            if not bool(guard_check.get("ok")):
                raise PermissionError(
                    str(guard_check.get("error") or "guard blocked read")
                )
        ...
        if step_type == "verify":
            ...
            if path:
                ...
                if not bool(read_guard.get("ok")):
                    raise PermissionError(
                        str(read_guard.get("error") or "guard blocked verify read")
                    )

                if not os.path.exists(full_path):
                    raise FileNotFoundError(
                        f"verify file not found: {full_path}"
                    )
            ...
            if contains:
                if contains not in target_text:
                    raise RuntimeError(
                        f"verify failed: '{contains}' not found"
                    )
            ...
            if equals is not None:
                ...
                if str(target_text).strip() != expected.strip():
                    raise RuntimeError(
                        f"verify failed: expected exact match "
                        f"'{expected}', got '{str(target_text).strip()}'"
                    )
    ...
    def _resolve_step_path(
        ...
        if normalized.startswith("workspace/shared/"):
            ...
            return os.path.abspath(
                os.path.join(shared_dir, relative_part)
            )
    ...
    def _resolve_read_path_with_fallback(
        ...
        shared_fallback = os.path.abspath(
            os.path.join(shared_dir, normalized)
        )
