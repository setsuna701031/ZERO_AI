...
def _zero_patch_operator_get_checkpoints_v20(cls):
    if not hasattr(cls, "get_session_checkpoints"):
        return
    if getattr(cls.get_session_checkpoints, "_zero_v20_patched", False):
        return
        ...
        try:
            ...
            if failed_step:
                ...
                for checkpoint in list(checkpoints or []):
                    ...
                    if not hasattr(checkpoint, "session_id"):
                        setattr(checkpoint, "session_id", session_id)
                ...
                if not exists:
                    normalized.append(
                        SimpleNamespace(
                            ...
                            step_id=failed_step,
                            status="failed",
                            metadata={"source": "operator_failed_checkpoint_v20"},
                            result={"ok": False},
                        )
                    )
    ...
    wrapped._zero_v20_patched = True
    cls.get_session_checkpoints = wrapped

for _name, _obj in list(globals().items()):
    if isinstance(_obj, type):
        _zero_patch_operator_get_checkpoints_v20(_obj)
'''
...
for path in TARGETS:
    ...
    if "def get_session_checkpoints" in text and "operator" in text.lower():
        marker = "ZERO_PATCH_OPERATOR_FAILED_CHECKPOINT_V20"
