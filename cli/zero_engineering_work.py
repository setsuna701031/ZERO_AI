...
def main(argv=None):
    ...
    try:
        if ns.cmd=='start':
            ...
            b=load_intake_bundle(_store(ns), _sid(ns)); evidence=b[STORE_FILES['evidence']]; repo_identity={'repository_id':b[STORE_FILES['intake']].get('repository_reference',{}).get('repository_id','default'),**evidence.get('repository_identity',{})}; form=create_formal_work_request_from_confirmed_specification(b[STORE_FILES['candidate']], b[STORE_FILES['confirmation']], repository_root_reference='.', repository_identity=repo_identity); out=persist_formalized(_store(ns), b[STORE_FILES['intake']]['intake_id'], form);
        elif ns.cmd=='resume':
