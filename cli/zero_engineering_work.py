...
def main(argv=None):
    ...
    for c in ('status','inspect','review','approval-summary','authorization-summary','preview','result','resume','completion-review-summary','verify-flow','intake-status','specification','clarification','formalize','start-confirmed','validate-change-package','change-package','execution-evidence','run-tests','build-multifile-plan','validate-multifile-plan','multifile-plan','validate-reproduction-request','reproduction-request','reproduction-result','test-failure-evidence','repair-candidate','iteration-status','build-repair-planning-intake','root-cause-hypothesis','impact-analysis','repair-strategy','build-patch-candidate','validate-patch-candidate','patch-candidate','build-patch-authoring-intake','authored-patch'):
        sub.add_parser(c)
