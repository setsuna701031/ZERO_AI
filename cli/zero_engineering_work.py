...
from core.engineering.engineering_governed_explicit_commit import STORE_FILES as COMMIT_STORE_FILES, admit_commit, build_commit_candidate, build_commit_preparation_intake, build_explicit_commit_request as build_governed_commit_request, execute_governed_commit, inspect_commit_state, resume_commit_state, verify_commit, verify_commit_diff
...
def main(argv=None):
    ...
    for name in ('prepare-commit','commit-candidate','validate-commit-candidate','admit-commit','execute-commit','commit-evidence','verify-commit'):
        sp=sub.add_parser(name); sp.add_argument('--workspace-root',default='.')
        if name=='commit-candidate': sp.add_argument('--message',required=True); sp.add_argument('--body',default='')
    for name in ('confirm-commit','reject-commit'):
        sp=sub.add_parser(name); sp.add_argument('json_file')
    ...
    try:
        ...
        elif ns.cmd=='prepare-commit':
            b=load_session_store(_store(ns),_sid(ns)); out=build_commit_preparation_intake(b[APPLY_STORE_FILES['review']],b[APPLY_STORE_FILES['result']],b[APPLY_STORE_FILES['verification']],b[AUTHZ_STORE_FILES['candidate']],b[APPLY_STORE_FILES['usage']],workspace_root=ns.workspace_root,session_id=_sid(ns)); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['preparation'],out)
        elif ns.cmd=='commit-candidate':
            b=load_session_store(_store(ns),_sid(ns)); out=build_commit_candidate(b[COMMIT_STORE_FILES['preparation']],commit_message=ns.message,commit_body=ns.body,workspace_root=ns.workspace_root); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['candidate'],out)
        elif ns.cmd=='validate-commit-candidate':
            b=load_session_store(_store(ns),_sid(ns)); out=verify_commit_diff(b[COMMIT_STORE_FILES['candidate']],b[COMMIT_STORE_FILES['preparation']],workspace_root=ns.workspace_root); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['diff_verification'],out)
        elif ns.cmd=='admit-commit':
            b=load_session_store(_store(ns),_sid(ns)); out=admit_commit(b[COMMIT_STORE_FILES['preparation']],b[COMMIT_STORE_FILES['candidate']],b[COMMIT_STORE_FILES['diff_verification']],b[APPLY_STORE_FILES['review']],b[APPLY_STORE_FILES['usage']]); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['admission'],out)
        elif ns.cmd in {'confirm-commit','reject-commit'}:
            b=load_session_store(_store(ns),_sid(ns)); raw=_load(ns.json_file); raw['decision']='rejected' if ns.cmd=='reject-commit' else 'confirmed'; out=build_governed_commit_request(b[COMMIT_STORE_FILES['candidate']],b[COMMIT_STORE_FILES['admission']],raw); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['request'],out)
        elif ns.cmd=='execute-commit':
            b=load_session_store(_store(ns),_sid(ns)); request,admission,out=execute_governed_commit(b[COMMIT_STORE_FILES['request']],b[COMMIT_STORE_FILES['admission']],b[COMMIT_STORE_FILES['candidate']],workspace_root=ns.workspace_root); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['request'],request); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['admission'],admission); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['evidence'],out)
        elif ns.cmd=='commit-evidence': out=load_session_store(_store(ns),_sid(ns)).get(COMMIT_STORE_FILES['evidence'],{'commit_status':'not_started'})
        elif ns.cmd=='verify-commit':
            b=load_session_store(_store(ns),_sid(ns)); out=verify_commit(b[COMMIT_STORE_FILES['evidence']],b[COMMIT_STORE_FILES['request']],workspace_root=ns.workspace_root); write_session_artifact(_store(ns),_sid(ns),COMMIT_STORE_FILES['verification'],out)
