...
from core.runtime.runtime_capability_controlled_activation_preparation import prepare_capability_controlled_activation
from cli.zero_capability_runtime_activation_eligibility import _finish
def run(argv=None):
 p=argparse.ArgumentParser();p.add_argument("--acceptance",required=True);p.add_argument("--prepared-at");p.add_argument("--output")
 try:a=p.parse_args(argv)
 except SystemExit as x:return {"error":"invalid_arguments"},int(x.code)
 try:v=json.loads(Path(a.acceptance).read_text(encoding="utf-8-sig"))
 except (OSError,UnicodeError,json.JSONDecodeError):return {"error":"invalid_json_input"},2
 r=prepare_capability_controlled_activation(v,prepared_at=a.prepared_at)
