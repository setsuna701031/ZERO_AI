...
def run(argv=None):
 ...
 for n in ("eligibility","review-request","readiness-closure"):p.add_argument("--"+n,required=True)
 try:a=p.parse_args(argv);return build(_read(a.eligibility),_read(a.review_request),_read(a.readiness_closure)),0
