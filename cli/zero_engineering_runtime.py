...
ACTIONS=("request","capability-admission","adapter-invocation","admission","session","phase","preview","analyze","propose","prepare","authorize","transaction","execute","resume","checkpoint","result","verify","evidence","closure","inspect","pipeline")
def main(argv=None):
    ...
    try:
        ...
        if action=="inspect": out={"schemas":SCHEMAS,"actions":list(ACTIONS),"default_pipeline_mode":"preview","execution_enabled_by_default":False}
            ...
            elif p.get("formal_persistence") is True: out=resume_formal_persistence_session(ns.session_root,p["session_id"])
            ...
            out=orchestrate_engineering_runtime(p,p.get("workspace_identity"),ns.workspace_root,ns.execute,confirmed,ns.session_root)
            if action=="adapter-invocation": out=out.get("invocation_result",out["result"])
            elif action not in ("pipeline","preview","analyze","propose","prepare","authorize","transaction","execute"): out=out.get(action.replace("-","_"),out)
        print(canonical_json(out)); return 2 if "error" in out or out.get("status") in ("invalid","not_admitted","not_registered","inactive","deprecated","ambiguous","adapter_mismatch","operation_unsupported","blocked","failed","rejected","not_closed","invalid_registry","invalid_request") else 0
