...
def main(argv=None):
    ap=argparse.ArgumentParser(); ap.add_argument("action",choices=ACTIONS); ap.add_argument("--json",default="{}"); ap.add_argument("--workspace-root"); ap.add_argument("--session-root"); ap.add_argument("--execute",action="store_true"); ap.add_argument("--confirm-controlled-execution",action="store_true"); ns=ap.parse_args(argv)
    try:
        ...
        if action=="inspect": out={"schemas":SCHEMAS,"actions":list(ACTIONS),"default_pipeline_mode":"preview","execution_enabled_by_default":False}
            ...
            out=orchestrate_engineering_runtime(p,p.get("workspace_identity"),ns.workspace_root,ns.execute,confirmed); out=out.get(action,out) if action not in ("pipeline","preview","analyze","propose","prepare","authorize","transaction","execute") else out
