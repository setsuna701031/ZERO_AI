...
def run(argv: list[str] | None = None) -> tuple[Any, int]:
    parser = argparse.ArgumentParser(prog="zero-capability-authorization-token-issuance-eligibility")
    parser.add_argument("--token", required=True)
    parser.add_argument("--evaluated-at")
    parser.add_argument("--output")
    try: args = parser.parse_args(argv)
    except SystemExit as exc: return {"error": "invalid_arguments"}, int(exc.code)
    try: value = json.loads(Path(args.token).read_text(encoding="utf-8-sig"))
    except (OSError, UnicodeError, json.JSONDecodeError): return {"error": "invalid_json_input"}, 2
    result = evaluate_capability_authorization_token_issuance_eligibility(value, evaluated_at=args.evaluated_at)
    ...
    if not validate_capability_authorization_token_issuance_eligibility(result).valid:
        return {"error": "invalid_issuance_eligibility"}, 2
