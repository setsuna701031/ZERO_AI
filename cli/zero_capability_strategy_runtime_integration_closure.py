...
from core.runtime.runtime_capability_strategy_runtime_integration_closure import SCHEMA,close_runtime_integration
...
def build_parser()->argparse.ArgumentParser:
 p=argparse.ArgumentParser(prog="python -m cli.zero_capability_strategy_runtime_integration_closure");s=p.add_subparsers(dest="command",required=True)
 for c in ("close","validate","inspect"):s.add_parser(c).add_argument("json_file")
 return p
def run(argv:list[str]|None=None)->tuple[Any,int]:
 ...
 try:
  ...
  if a.command=="close":
   out=close_runtime_integration(value);return out,0 if out["status"]=="closed" else 1
  if not isinstance(value,dict) or value.get("schema")!=SCHEMA:return {"valid":False,"errors":["unsupported_schema"]},1
  v=validate_runtime_integration_closure(value)
  if a.command=="validate":return {"valid":v.valid,"errors":list(v.errors)},0 if v.valid else 1
  return {"valid":v.valid,"schema":value.get("schema"),"status":value.get("status"),"closure_id":value.get("closure_id"),"source_verification_id":value.get("source_verification_id"),"verification_closed":value.get("boundary",{}).get("verification_closed")},0 if v.valid else 1
