...
from core.engineering.repository_analysis_request import SCHEMA,prepare_repository_analysis_request
...
def run(argv=None):
 ...
 try:
  v=_read(a.json_file)
  if a.command=="prepare":o=prepare_repository_analysis_request(v);return o,0 if o["status"] in {"prepared","needs_clarification"} else 1
  if not isinstance(v,dict) or v.get("schema")!=SCHEMA:return {"valid":False,"errors":["unsupported_schema"]},1
  x=validate_repository_analysis_request(v);return ({"valid":x.valid,"errors":list(x.errors)} if a.command=="validate" else {"valid":x.valid,"schema":v.get("schema"),"status":v.get("status"),"repository_analysis_request_id":v.get("repository_analysis_request_id")}),0 if x.valid else 1
