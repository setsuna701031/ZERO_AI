...
A verified local commit may now enter a separately governed remote-mutation chain: Verified Commit Closure, Push Preparation, read-only Remote Verification, Human Push Review, Explicit Push Authorization, exact-SHA Governed Push, append-only Push Evidence, post-push Remote Verification, and Push Closure. Verified Commit Closure requires canonical Commit Verification with `verification_status == verified` and complete canonical Commit Evidence; SHA agreement alone is insufficient. Its immutable `commit_verification_closure_id` is copied and validated at every later stage—never rediscovered or replaced. The canonical preparation binds this sealed closure plus repository ID, remote name and URL, branch, full commit/parent/tree SHAs, Commit Verification fingerprint, and a stable push fingerprint shared by every later decision.

Preflight requires an existing remote branch whose head is exactly the target commit's parent, proves fast-forward eligibility, and permits exactly one commit. Immediately before execution ZERO revalidates only the sealed closure selected by Push Preparation—its canonical integrity, immutable ID, `verified` status, and complete-evidence seal—plus actual commit parent/tree, local HEAD and branch, clean workspace, remote URL, and frozen remote HEAD. It never rediscovers or replaces the source Verification/Evidence artifacts. The only constructed mutation is `git push <remote> <40-char SHA>:refs/heads/<branch>`. `HEAD` sources, force options, tags, deletion refspecs, wildcards, multiple commits, branch creation, retry, pull, merge, rebase, PR, release, and workflow operations are unavailable.

## v5.6 Governed Explicit Pull Request Review

A sealed successful Push Closure may now enter a separately governed provider-neutral PR chain: PR Preparation, Repository Provider Admission, read-only Remote Branch Verification, Human PR Review, Explicit PR Authorization, Creation Request, one-attempt PR Execution, Evidence, post-creation verification, and sealed PR Closure. The immutable `push_closure_id` is the sole upstream governance identity carried by every downstream artifact; raw Push Evidence, latest-closure lookup, closure substitution, and copied push review/authorization status are not accepted.

Preparation binds an exact repository provider/identity/remote, distinct validated source and target branches, exact source and target heads, bounded title/body fingerprints, and the pushed commit. Review and PR Authorization are separate explicit human decisions and freeze both remote heads. The executor revalidates the sealed Push Closure, all canonical references, authorization scope and single-use state, current branch heads, source ancestry, changes, and equivalent-PR absence before one structured provider call. Post-verification proves the created PR is open, unmerged, unclosed, and matches the authorized repository, branches, and source commit. Successful closure stops at `awaiting_merge_review`; merge, automatic PR creation, retry, labels, reviewers, milestones, branches, tags, releases, issues, and workflows are unavailable.

## v5.7 Governed Explicit Merge Review

A sealed PR Closure in `awaiting_merge_review` may now enter a separately governed provider-neutral merge chain: Merge Preparation, read-only Remote Verification, deterministic Eligibility, Human Merge Review, Explicit Merge Authorization, Execution Request, one merge attempt, Evidence, mandatory post-merge verification, and sealed Merge Closure. The immutable `pr_closure_id` is the sole upstream identity carried by every merge artifact; no latest PR/closure lookup, raw evidence fallback, PR approval reuse, automatic PR selection, or strategy selection is allowed.

v5.7 supports exactly `merge_commit`. Preparation and review freeze the repository, PR, source/target branches and heads, source commit, and method. Execution revalidates the sealed PR Closure, canonical preparation, eligibility, separate review and single-use authorization, open/unmerged PR state, both current heads, remaining difference, and clean conflict status before one structured provider call. Entering that call consumes the authorization even when the provider fails, times out, raises, or returns ambiguity; another attempt requires a new review and authorization. Provider success is insufficient: post-verification must prove the PR is merged, the merge commit is the target head, the source is reachable, the source branch still exists, and no unrelated branch changed. Successful closure is `merged_verified`; auto-merge, merge queue, retries, squash/rebase, branch deletion, tags, releases, deployments, and workflows are unavailable.
