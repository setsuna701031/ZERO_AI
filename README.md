...
A verified local commit may now enter a separately governed remote-mutation chain: Push Preparation, read-only Remote Verification, Human Push Review, Explicit Push Authorization, exact-SHA Governed Push, append-only Push Evidence, post-push Remote Verification, and Push Closure. The canonical preparation binds repository ID, remote name and URL, branch, full commit/parent/tree SHAs, Commit Verification fingerprint, and a stable push fingerprint shared by every later decision.

Preflight requires an existing remote branch whose head is exactly the target commit's parent, proves fast-forward eligibility, and permits exactly one commit. Immediately before execution ZERO revalidates the original Commit Verification and Evidence artifacts, actual commit parent/tree, local HEAD and branch, clean workspace, remote URL, and frozen remote HEAD. The only constructed mutation is `git push <remote> <40-char SHA>:refs/heads/<branch>`. `HEAD` sources, force options, tags, deletion refspecs, wildcards, multiple commits, branch creation, retry, pull, merge, rebase, PR, release, and workflow operations are unavailable.
