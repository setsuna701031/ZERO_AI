# ZERO Demo: Event → Engineering Automation

## What this demo shows

- File event triggers task
- Task routed automatically
- Tool generates real engineering artifacts
- Full audit + execution tracking
...
```text
Event → Task → ToolRouter → Tool → Audit → ExecutionSession → Outbox
```

The demo creates `workspace/events_inbox/issue_demo.txt`, runs one event poll, routes the task through the existing runtime path, writes GitHub workflow artifacts locally, records audit rows, and saves an execution session.
...
- `workspace/events_outbox/event_results.jsonl`
- `workspace/execution_sessions/<session_id>.json`
- `workspace/audit_logs/tool_audit.jsonl`

No GitHub API call, `git commit`, or `git push` is performed.
