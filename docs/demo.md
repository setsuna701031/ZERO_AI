...
2. `workspace/shared/numbers\\\_input.txt`
3. the command:
...
* `number\\\_stats.py` exists
* `stats\\\_result.txt` exists
...
* `number\\\_stats.py` is created
* the script runs successfully
* `stats\\\_result.txt` is written
* the result contains:
...
* `workspace/shared/document\\\_flow\\\_trace.json`
...
* `test\\\_step\\\_executor.py`
* `test\\\_executor\\\_repair\\\_rules.py`
* `test\\\_executor\\\_safe\\\_path\\\_repair.py`
* `run\\\_agent\\\_loop\\\_smoke.py`
* `run\\\_scheduler\\\_smoke.py`
