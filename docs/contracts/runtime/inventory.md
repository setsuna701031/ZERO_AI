...
| Runtime Recovery Controlled Activation Authorization Effect Blocker | TBD | core/runtime/recovery_controlled_activation_authorization_effect_blocker_policy.py | tests/test_recovery_runtime_controlled_activation_authorization_effect_blocker_bundle.py | Missing Spec | Packages 425-432 authorization effect blocker remains disabled, data-only, blocker-status-record-only, and non-mutating; package definitions did not list a dedicated contract spec file |
