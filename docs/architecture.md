...
The remote-mutation chain is: verified Commit Closure -> Push Preparation -> read-only Remote Verification -> Human Push Review -> Explicit Push Authorization -> exact-SHA Push Execution -> append-only Push Evidence -> post-push Remote Verification -> Push Closure. Each transition uses canonical references to one preparation fingerprint; review is not authorization, and authorization is single-use.
