...
The PR chain is: sealed Push Closure -> PR Preparation -> Provider Admission -> Remote Branch Verification -> Human PR Review -> Explicit PR Authorization -> Creation Request -> Provider Execution -> Evidence -> post-creation verification -> PR Closure. Push Closure is the only trusted upstream artifact. Every PR artifact carries exactly the same `push_closure_id`; the runtime resolves no latest closure, accepts no Push Evidence substitute, and copies no upstream approval or authorization state.

The governance core depends on a narrow provider-neutral adapter protocol for structured repository inspection, branch inspection, equivalent-open-PR lookup, one PR creation, and PR inspection. Provider adapters receive no free-form commands or arbitrary arguments. Preparation and pre-execution validation bind repository identity/URL, exact source and target heads, source commit ancestry and difference, bounded title/body fingerprints, canonical review and authorization references, and a one-attempt scope. Any branch drift requires new preparation and review. Closure is sealed only after provider evidence and read-only verification prove an open, unmerged, unclosed PR matching the authorized identity; its only successful state is `awaiting_merge_review`.

## v5.7 Governed Explicit Merge Review

The merge chain is: sealed PR Closure -> Merge Preparation -> Remote Verification -> Eligibility -> Human Merge Review -> Explicit Merge Authorization -> Execution Request -> one Provider Merge -> Evidence -> post-verification -> Merge Closure. PR Closure is the sole upstream governance artifact and one immutable `pr_closure_id` flows through every stage. Eligibility proves facts but grants no approval; PR review and PR authorization grant no merge authority.

The provider-neutral merge adapter exposes only structured PR/branch inspection, one merge operation, and merge-result inspection. Only `merge_commit` is admitted. Source and target heads are frozen before review and checked immediately before execution; drift invalidates the full downstream chain. Authorization permits one exact provider/repository/PR/head/method tuple and is consumed on the single attempt. A `merged_verified` closure requires read-only proof of merge state, merge commit identity, target-head equality, source reachability, preserved source branch, and no unrelated branch change. The closure explicitly denies release, tag, deployment, and source deletion claims.
